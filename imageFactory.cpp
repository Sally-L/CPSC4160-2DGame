...
ImageFactory::~ImageFactory() {
  ...
  // Convert these three into while loops without auto
  std::map<std::string, SDL_Surface*>::const_iterator ptrS = surfaces.begin();
  while ( ptrS != surfaces.end() ) {
    SDL_FreeSurface(ptrS->second);
    ptrS++;
  }
  std::map<std::string, SDL_Texture*>::const_iterator ptrT = textures.begin();
  while ( ptrT != textures.end() ) {
    SDL_DestroyTexture(ptrT->second);
    ptrT++;
  }
  std::map<std::string, Image*>::const_iterator ptrI = images.begin();
  while ( ptrI != images.end() ) {
    std::cout << "deleting " << ptrI->first << std::endl;
    delete ptrI->second;
    ptrI++;
  }
