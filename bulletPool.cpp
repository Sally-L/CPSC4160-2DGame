...
bool BulletPool::collidedWith(const Drawable* obj) const {
  ...
  while (ptr != bulletList.end()) {
    if ( strategy->execute(*ptr, *obj) ) {
      if(const SmartSprite* smartObj = dynamic_cast<const SmartSprite*>(obj)) {
        if (!(smartObj->isExploding())){
...
void BulletPool::shoot(const Vector2f& position, const Vector2f& velocity, bool goingLeft) {
	if (timeSinceLastFrame > frameInterval) {
    // If no bullets in pool, make one:
    if ( freeList.empty() ) {
      Bullet b(name, goingLeft);
