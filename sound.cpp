  ...
  channels() 
{
  ...
  music = Mix_LoadMUS("sound/background_hedwig_theme.mp3");
  // Need to install midi to play the following:
  // music = Mix_LoadMUS("sound/ballad2.mid");
  if (!music) throw string("Couldn't load background_hedwig_theme.mp3")+Mix_GetError();

  startMusic();

  sounds.push_back( Mix_LoadWAV("sound/expectoFast1try.wav") );
  sounds.push_back( Mix_LoadWAV("sound/death.wav") );
  sounds.push_back( Mix_LoadWAV("sound/goGryffindor.wav") );
  sounds.push_back( Mix_LoadWAV("sound/pointsForGryf.wav") );
  sounds.push_back( Mix_LoadWAV("sound/captureSnitchtry.wav") );
  sounds.push_back( Mix_LoadWAV("sound/DEMENTOR.wav") );
  sounds.push_back( Mix_LoadWAV("sound/dementorDeathtry.wav") );
